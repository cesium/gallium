defmodule GalliumWeb.UserAuth do
  @moduledoc """
  Handles the authentication lifecycle and session management for users.
  """
  ...
  @remember_me_cookie "_gallium_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      GalliumWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      GalliumWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule GalliumWeb.PageLive do
        use GalliumWeb, :live_view

        on_mount {GalliumWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{GalliumWeb.UserAuth, :require_authenticated}] do
  ...
  @doc """
  Used for routes that should not be accessed by authenticated users.
  """
